...
internal sealed class GetTags : IEndpoint
{
    ...
    {
        app.MapGet("users/tags", async (ClaimsPrincipal claims, ISender sender) =>
        {
            Result<IReadOnlyCollection<TagGroupResponse>> result = await sender.Send(new GetUsersInterestsQuery());
